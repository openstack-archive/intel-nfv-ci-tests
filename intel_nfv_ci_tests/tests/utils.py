...
from tempest.lib import exceptions as t_exceptions
...
def get_host_libvirt_uri(hvs_client, servers_admin_client, server_id):
    ...
    return 'qemu+ssh://%s@%s/system?no_verify=1&keyfile=%s' \
           % (CONF.intel_nfv_ci.qemu_ssh_user, hv_ip,
              CONF.intel_nfv_ci.qemu_ssh_private_key_path)


def get_host_ssh_client(hvs_client, servers_admin_client, server_id):
    if (
        CONF.intel_nfv_ci.qemu_ssh_user is None or
        CONF.intel_nfv_ci.qemu_ssh_private_key_path is None
    ):
        message = '[intel_nfv_ci] section does not contain qemu_ssh_user ' \
                  'or qemu_ssh_private_key_path attributes'
        raise t_exceptions.InvalidConfiguration(message)
