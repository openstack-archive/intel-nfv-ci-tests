...
class TestServerNumaBase(manager.NetworkScenarioTest):
    ...
    def create_flavor_with_numa(self, numa_nodes=2, ram=2048, vcpus=4, disk=0):
    ...
    def verify_ssh(self):
        ...
        self.floating_ips_client.associate_floating_ip_to_server(
            floating_ip['floating_ip']['ip'], self.instance['id'])
        # Check ssh
        return self.get_remote_client(
            server_or_ip=floating_ip['floating_ip']['ip'],
            username='cirros',
    ...
    def get_numa_nodes(self):
        ...
        ret = [str(x * cpu_per_soc) + '-' + str(x * cpu_per_soc + cpu_per_soc -
               1) + ',' + str(x * cpu_per_soc + 2 * cpu_per_soc) + '-' +
...
class TestServerNumaPCI(TestServerNumaBase):
    ...
    def boot_instance(self, flavor=None):
        if flavor is None:
            self.create_flavor_with_numa()
        # Create server with image and flavor from input scenario

        net_id = self._get_network_by_name(CONF.compute.network_for_ssh)['id']
        create_port_body = {'binding:vnic_type': 'direct',
                            'namestart': 'port-sriov'}
        ...
        self.instance = super(manager.NetworkScenarioTest, self).create_server(
            image=self.image_ref, flavor=flavor,
    ...
    def test_server_numa_pci(self):
        ...
        sriov_port = self._list_ports(device_id=self.instance['id'])[0]
        pci_slot = sriov_port['binding:profile']['pci_slot']
        pci_node = self.get_pci_numa_node(pci_slot)
